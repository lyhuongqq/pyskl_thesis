...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def label2array(num, label):
        print(f"label: {label}, type: {type(label)}")  # Debugging output
        if num is None:
           raise ValueError("num_classes is not defined")
        # Ensure label is a list or array-like, not a scalar
        if isinstance(label, int):
           #label = [label]  # Convert scalar to list
        #elif isinstance(label, list) and len(label) == 0:
        #   raise ValueError(f"Invalid label format: {label}")  # Handle empty list case
           arr = np.zeros(num, dtype=np.float32)
           arr[label] = 1.
        else:
        # Handle unexpected types of labels
           raise ValueError(f"Expected an integer label, but got {type(label)}")
        return arr
    def evaluate(self,
                 results,
        ...
        # Debug: print gt_labels and num_classes
        print(f"gt_labels: {gt_labels}, num_classes: {self.num_classes}")
        
        # Filter out invalid labels (like None or empty lists)
        #gt_labels = [label for label in gt_labels if label is not None and len(label) > 0]
        gt_labels = [label for  label in gt_labels if label is not None]
