    ...
    def resample_annotations(annotations, synthetic=False):
        ...
        for i, (feature, label) in enumerate(zip(resampled_features, resampled_labels)):
            ...
            resampled_annotations.append({
                'frame_dir': str(frame_dir),  # Keep the original name for original data
                'total_frames': total_frames,
                'img_shape': meta['img_shape'],
                'original_shape': meta['original_shape'],
                'label': label,
                'keypoint': reshaped_keypoints,
                'keypoint_score': np.zeros_like(reshaped_keypoints[..., 0]) if label == 5 else np.ones_like(reshaped_keypoints[..., 0])
            })
...
resample_pickle_data(
    input_file=r"D:\pyskl-main\aug_8_modified_file.pkl",
    train_output_file=r"D:\pyskl-main\smotetomek_aug_8_train5.pkl",
    val_output_file=r"D:\pyskl-main\smotetomek_aug_8_val.pkl"
)
